...
**`gdis` now implements this extraction family-aware** (auto-detected, or forced
with `--iso`). It emits cols 1–76 (COLBIN) of every card whose Hollerith
identifier (cols 77–79) decodes cleanly and progresses as the isolation deck's
card number, concatenated in deck order at `--org`, and skips cards with a
blank/garbled identifier (a stray title/summary/blank separator) rather than
assuming fixed first/last positions — the captured decks already have the title
